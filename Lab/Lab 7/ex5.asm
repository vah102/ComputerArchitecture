...
main: 	
	...
	nop
print_max:	
	li 	$v0, 4 		#code for print to console
	la 	$a0, message_max
	syscall
	li 	$v0, 1
...
#-----------------------------------
push: 	
	addi 	$sp, $sp, -32 	#stack chua 8 ptu
	sw 	$s0, 28($sp)
	sw 	$s1, 24($sp)
	sw 	$s2, 20($sp)
	sw 	$s3, 16($sp)
...
#----------------------------------------------
max: 	
	lw 	$v1, 0($sp)		#pop phan tu dau tien vao $v1
	addi 	$sp, $sp, 4
	...
	j 	max
max_end:	
	add 	$sp, $sp, -32 	#adjust stack pointer to the top of stack
	jr 	$ra
#----------------------------------------------
min: 	
	lw 	$v1, 0($sp)		#pop tu stack
	addi 	$sp, $sp, 4
